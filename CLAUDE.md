...
6. **Recurring tasks are "jobs", not workflows.** Two shapes, both ride the existing `templates/kody.yml` cron wake — neither requires any YAML:
   - **Watch executable** (engine-shipped): add `src/executables/<name>/profile.json` with `role: "watch"`, `kind: "scheduled"`, and a `schedule` cron. [`dispatchScheduledWatches`](src/dispatch.ts) fans it out on each wake. Output via `postIssueComment`, never `commitAndPush`. Canonical example: [src/executables/watch-stale-prs/](src/executables/watch-stale-prs/).
   - **Markdown job** (consumer-owned): drop `.kody/jobs/<slug>.md` in the consumer repo. Ticked by the existing `job-scheduler` watch — contract in [src/executables/job-tick/prompt.md](src/executables/job-tick/prompt.md).
